...
task main()
{
   ...
   DriveByButtonSpeed = MotorSpeed;
   GyroSensitivity = 4; // a little less sensistive, because we're using many sensors

   int oldState = LineState();
   GoStraight(true);
   int previousAction = AGOSTRAIGHT;
   int action = AGOSTRAIGHT;
   float straightHead = currHeading;
   wait1Msec(500);

   // follow line until 1. we decide to stop, or 2. we have swiveled 180 deg. looking for line
   while ((action != ASTOP) && (abs(SubtractFromCurrHeading(straightHead)) < 100)) {
     int newState = LineState();
     action = DetermineAction (oldState, newState, previousAction);
     nxtDisplayTextLine(1, "head: %3.0f", currHeading);
     nxtDisplayTextLine(2, "%d", abs(SubtractFromCurrHeading(straightHead)));
     nxtDisplayTextLine(4, "%d", oldState);
     nxtDisplayTextLine(5, "%d", newState);
     nxtDisplayTextLine(6, "%d", action);
     switch (action) {
       case AGOSTRAIGHT: GoStraight(true); LightBothLeds(); straightHead = currHeading; break;
       case ATURNRIGHT: RotateRight(MotorSpeed); LightLed(1); break;
       case ATURNLEFT: RotateLeft(MotorSpeed); LightLed(2); break;
       case ASTOP: break;
     } // switch
